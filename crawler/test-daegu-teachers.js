/**
 * 대구광역시교육청 기간제교사·강사 크롤러 테스트
 * URL: https://www.dge.go.kr/main/na/ntt/selectNttList.do?mi=5186&bbsId=1047
 */
...
async function main() {
  console.log('🚀 대구광역시교육청 기간제교사·강사 크롤러 테스트 시작\n');
  ...
  try {
    ...
    const config = sourcesConfig['daegu-teachers'];
    ...
    const jobs = await crawlNttPattern(page, config);
    ...
    if (jobs.length > 0) {
      ...
      jobs.forEach((job, i) => {
        ...
        if (job.hasContentImages) {
          console.log(`    🖼️  본문 이미지: 있음`);
        }
