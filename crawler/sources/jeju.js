...
/**
 * 제주특별자치도교육청 채용공고 크롤러
 * URL: https://www.jje.go.kr/index.jje (채용·시험정보)
 */
export async function crawlJeju(page, config) {
  console.log(`\n📍 ${config.name || '제주특별자치도교육청'} 크롤링 시작`);

  const jobs = [];
  const baseUrl = 'https://www.jje.go.kr';
  // 채용정보 게시판
  const listUrl = `${baseUrl}/board/list.jje?boardId=BBS_0000017&menuCd=DOM_000000103001001000`;
