...
export async function generateCrawlerCode(boardName: string, boardUrl: string): Promise<string> {
  ...
  lines.push('  ]')
  lines.push('')
  lines.push('  const contentSelectors = [')
  lines.push("    '.board-view',")
  lines.push("    '.view-content',")
  lines.push("    '.content',")
  lines.push("    '.detail',")
  lines.push("    '#content',")
  lines.push("    '.post-content',")
  lines.push("    '.article-content',")
  lines.push("    'td.td-left',")
  lines.push("    '.board_view',")
  lines.push("    '.view_con',")
  lines.push("    '.con',")
  lines.push('  ]')
  lines.push('')
  ...
  lines.push('')
  lines.push('        // Fetch detail page content')
  lines.push('        let detailContent = ""')
  lines.push('        try {')
  lines.push("          await page.goto(finalUrl, { waitUntil: 'domcontentloaded', timeout: 15000 })")
  lines.push('          await page.waitForTimeout(500)')
  lines.push('')
  lines.push('          for (const selector of contentSelectors) {')
  lines.push('            const contentLoc = page.locator(selector).first()')
  lines.push('            if (await contentLoc.count()) {')
  lines.push('              const text = await contentLoc.textContent()')
  lines.push('              if (text && text.trim().length > 50) {')
  lines.push('                detailContent = text.trim()')
  lines.push("                console.log('[INFO] content extracted via:', selector)")
  lines.push('                break')
  lines.push('              }')
  lines.push('            }')
  lines.push('          }')
  lines.push('')
  lines.push('          // Go back to list page')
  lines.push("          await page.goBack({ waitUntil: 'domcontentloaded', timeout: 10000 })")
  lines.push('          await page.waitForTimeout(500)')
  lines.push('        } catch (detailErr) {')
  lines.push("          console.warn('[WARN] failed to fetch detail for:', titleResult.value)")
  lines.push('          // Navigate back to list page if stuck')
  lines.push('          try {')
  lines.push("            await page.goto(target, { waitUntil: 'domcontentloaded', timeout: 10000 })")
  lines.push('            await page.waitForTimeout(500)')
  lines.push('          } catch (e) {')
  lines.push("            console.error('[ERROR] failed to return to list page')")
  lines.push('          }')
  lines.push('        }')
  lines.push('')
