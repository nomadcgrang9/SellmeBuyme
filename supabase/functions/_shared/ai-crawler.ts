...
export async function generateCrawlerCode(boardName: string, boardUrl: string): Promise<string> {
  ...
  const lines: string[] = []
  lines.push('/**')
  lines.push(' * ' + boardName + ' crawler (AI minimal + heuristics)')
  ...
  lines.push("    const t = txt.replace(/[\\.\\/]/g, '-').trim()")
  lines.push('    // Prefer YYYY-MM-DD shape if present')
  lines.push("    const m = t.match(/(\\d{4})-(\\d{1,2})-(\\d{1,2})/)")
  lines.push('    if (m) {')
  lines.push('      const y = m[1]')
  lines.push('      const mm = ("0" + m[2]).slice(-2)')
  lines.push('      const dd = ("0" + m[3]).slice(-2)')
  ...
  lines.push('')
  lines.push('  if (rows.length === 0) {')
  lines.push("    console.warn('[WARN] no rows found with fallback selectors')")
  ...
  lines.push('  }')
  lines.push('')
  lines.push('  // Detect common pagination controls (logging only)')
  lines.push('  try {')
  ...
  lines.push("        console.log('[INFO] pagination candidate detected:', psel)")
  lines.push('        break')
  lines.push('      }')
  lines.push('    }')
  lines.push('  } catch { /* ignore */ }')
  lines.push('')
  lines.push("  console.log('[INFO] collected', jobs.length)")
  lines.push('  return jobs')
  lines.push('}')
  return lines.join('\n')
}
