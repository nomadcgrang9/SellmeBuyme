...
export const JobDetailPanel: React.FC<JobDetailPanelProps> = ({
  ...
  const isBookmarked = job ? isBookmarkedFn(job.id) : false;

  const handleBookmarkToggle = useCallback(async () => {
    if (!user || !job) { showToast('로그인이 필요합니다', 'error'); return; }
    ...
    try {
      ...
      console.error('[JobDetailPanel] 북마크 토글 실패:', err);
      if (isBookmarked) addToStore(job.id); else removeFromStore(job.id);
      showToast('오류가 발생했습니다', 'error');
    } finally {
      setIsBookmarkLoading(false);
    }
  }, [user, job, isBookmarked, isBookmarkLoading, addToStore, removeFromStore, showToast]);
      ...
      <div className="px-4 py-3 border-b border-gray-100 flex-shrink-0">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            ...
            >
              <svg className="w-5 h-5" viewBox="0 0 24 24" fill={isBookmarked ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth={isBookmarked ? 0 : 2}>
