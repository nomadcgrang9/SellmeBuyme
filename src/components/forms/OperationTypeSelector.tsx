...
export default function OperationTypeSelector({ value, onChange, error }: OperationTypeSelectorProps) {
  const [isOpen, setIsOpen] = useState(false);

  const getDisplayText = () => {
    if (value.length === 0) return '운영 방식 선택';
  ...
  return (
    <div className="space-y-0.5 relative">
      <label className="text-[14px] font-semibold text-gray-700 block mb-0.5">
        운영 방식 <span className="text-red-500">*</span>
      </label>
        ...
        <div className="absolute top-full left-0 right-0 mt-0.5 bg-white border border-gray-300 rounded-lg shadow-lg z-20 p-2">
          <div className="space-y-1">
            {OPERATION_TYPE_OPTIONS.map(type => (
              <label key={type} className="flex items-center gap-1.5 hover:bg-gray-50 p-1 rounded cursor-pointer">
                <input
                  type="checkbox"
