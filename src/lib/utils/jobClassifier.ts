 ...
 * 2차/3차 필터 매칭 로직 포함
 * 시설/환경에서 배움터지킴이 제외 (안전지킴이로 분류되도록)
 */
...
export const SECONDARY_OPTIONS: Partial<Record<PrimaryCategory, { key: string; label: string }[]>> = {
  ...
  '행정·교육지원': [
    ...
    { key: '과학/정보실무사', label: '과학/정보실무사' },
    { key: '특수교육실무사', label: '특수교육실무사' },
    { key: '돌봄전담사', label: '돌봄전담사' },
    { key: '교육행정', label: '교육행정' },
  ],
};
...
export function classifyJob(job: JobLike): PrimaryCategory {
  ...
  // 공무직: 교육공무직, 공무직, 무기계약, 교무행정, 과학실무, 정보실무, 특수교육실무, 돌봄전담
  // 행정직: 교육행정, 행정직, 지방공무원
  const adminJobKeywords = ['시설', '미화', '운영직', '경비', '조리', '당직', '청소',
    '보조원', '실무사', '급식', '배식', '청원경찰',
    // 공무직 키워드 확장
    '교육공무직', '무기계약', '과학실무', '정보실무', '특수교육실무', '돌봄전담',
    // 교육행정직 키워드
    '교육행정', '행정직', '지방공무원'];
  ...
  if (isSecondary) {
    ...
    if (
      ...
      tl.includes('보호인력') || tl.includes('교육복지') || tl.includes('배움터') ||
      tl.includes('운전') || tl.includes('통학') || tl.includes('미화') || tl.includes('경비')
    ) {
      return '행정·교육지원';
    }
    return '교과과목';
...
// v9: 조리/교무 구분 강화 - 상호 배타적 키워드 처리 및 엄격한 태그 매칭
function matchesAdmin(titleLower: string, tagsLower: string[], category: string): boolean {
  // 디버그: 교육행정 필터 호출 시 로그
  if (category === '교육행정') {
    console.log('[matchesAdmin] 호출됨, category:', category, 'title:', titleLower.substring(0, 50));
  }
  const keywords: Record<string, string[]> = {
    '교무실무사': ['교무실무', '교무행정', '교무지원', '행정보조', '행정실무사', '사무행정'],
    '조리실무사': ['조리실무', '조리사', '조리원', '급식', '배식', '조리종사'],
    '시설/환경': ['시설', '환경', '관리원', '미화', '경비', '청소', '당직'],
    '영양사': ['영양사'],
    '학습튜터/협력강사': ['튜터', '협력강사', '학습지원', '찬찬', '학습보조'],
    '자원봉사': ['봉사', '자원봉사'],
    '안전지킴이': ['지킴이', '안전', '교통', '보호인력', '학생보호'],
    // 공무직/교육행정 확장
    '과학/정보실무사': ['과학실무', '정보실무', '과학실', '전산실무', '컴퓨터실무'],
    '특수교육실무사': ['특수교육실무', '특수실무', '장애학생', '통합교육실무'],
    '돌봄전담사': ['돌봄전담', '초등돌봄', '돌봄교실', '늘봄전담'],
    // 교육행정: 교육공무직/공무직 제거 (모든 공고에 매칭되는 문제 방지)
    '교육행정': ['교육행정', '행정직', '지방공무원 대체직원', '사무직원'],
  };

  // 상호 배타적 카테고리 정의 (각 카테고리는 다른 전문직종 키워드 포함 시 제외)
  const exclusiveCategories: Record<string, string[]> = {
    '교무실무사': ['조리', '급식', '배식', '조리사', '조리원', '영양사', '미화', '시설관리', '당직', '특수교육실무', '돌봄전담', '과학실무'],
    '조리실무사': ['교무', '행정보조', '사무행정', '교무지원', '영양사', '특수교육실무'],
    '영양사': ['조리실무', '조리사', '조리원', '교무실무', '교무행정'],
    '시설/환경': [
      '조리', '급식', '배식', '조리사', '조리원', '영양사',  // 조리실무사/영양사 제외
      '동화구연', '미디어체험', '강사',                      // 강사 제외
      '특수교육실무', '돌봄전담',                             // 특수교육/돌봄 제외
      '지킴이', '배움터', '안전', '보호인력', '학생보호',    // 안전지킴이 제외 (배움터지킴이 포함)
    ],
    // 학습튜터/협력강사: 특수교육실무사 등 다른 직종 제외
    '학습튜터/협력강사': [
      '특수교육실무', '특수실무', '장애학생',        // 특수교육실무사 제외
      '돌봄전담', '늘봄전담', '돌봄교실',           // 돌봄전담사 제외
      '과학실무', '정보실무', '전산실무',           // 과학/정보실무사 제외
      '조리', '급식', '배식',                        // 조리실무사 제외
      '시설', '미화', '경비', '당직',               // 시설/환경 제외
      '영양사',                                      // 영양사 제외
    ],
    // 안전지킴이: 다른 직종 제외
    '안전지킴이': [
      '특수교육실무', '특수실무',                    // 특수교육실무사 제외
      '조리', '급식', '배식',                        // 조리실무사 제외
      '돌봄전담', '늘봄전담',                        // 돌봄전담사 제외
    ],
    // 특수교육실무사: 다른 직종 제외
    ...
    // 교육행정: 다른 전문직종 제외 (공무직이라는 단어만으로 매칭되지 않도록)
    '교육행정': [
      '조리', '급식', '배식', '조리사', '조리원', '조리실무',   // 조리실무사 제외
      '영양사',                                      // 영양사 제외
      '시설', '미화', '경비', '당직', '관리원', '환경실무', '환경', // 시설/환경 제외
      '돌봄전담', '늘봄전담', '돌봄교실', '늘봄실무', '초등돌봄', '늘봄', '방과후전담', // 돌봄전담사/방과후 제외
      '과학실무', '과학실', '정보실무', '전산실무', '실습실무', // 과학/정보실무사 및 실습 제외
      '특수교육실무', '특수실무', '장애학생', '특수교육치료', '특수교육지도', // 특수교육 관련 모두 제외
      '튜터', '협력강사', '학습지원',               // 학습튜터/협력강사 제외
      '지킴이', '보호인력', '학생보호',             // 안전지킴이 제외
      '통학보조', '통학차량',                        // 통학 관련 제외
      '보조교사',                                    // 보조교사 제외 (초등담임으로 분류)
      '사서',                                        // 사서 제외 (비교과로 분류)
    ],
  };

  // 배타적 키워드가 타이틀에 있으면 해당 카테고리에서 제외
  const exclusiveKws = exclusiveCategories[category];
  if (exclusiveKws && exclusiveKws.some(kw => titleLower.includes(kw))) {
    // 디버그: 배제된 공고 로그
    if (category === '교육행정' && titleLower.includes('조리')) {
      console.log('[matchesAdmin] 교육행정에서 배제됨:', titleLower);
    }
    return false;
  }
  ...
  // title에서 매칭
  if (kws.some(kw => titleLower.includes(kw))) return true;
  // tags에서 엄격한 매칭 (태그가 키워드를 포함하는 경우만, 역방향 제외)
  return tagsLower.some(tag =>
    kws.some(kw => tag.includes(kw))
  );
}
